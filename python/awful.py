...
def BOS(e, c):
    x, e, c = eval_term(e, c)
    if x[0] != "LIST":
        raise BaseException("Parameter of BOS must be a stack")
...
def ISNIL(e, c):
    x, e, c = eval_term(e, c)
    return ["NUMBER", float(x[0] == "LIST" and x[1] == [])], e, c
...
def PUSH(e, c):
    ...
    x2, e, c = eval_term(e, c)
    if x2[0] != "LIST":
        raise BaseException("Second parameter of PUSH must be a stack")
...
def TOS(e, c):
    x, e, c = eval_term(e, c)
    if x[0] != "LIST":
        raise BaseException("Parameter of TOS must be a stack")
    if len(x[1]) == 0:
        raise BaseException("TOS applied to empty stack")
    return ["LIST", []] if len(x[1]) == 0 else x[1][0], e, c

KEYWORDS = {
    "ADD": ADD, "BOS": BOS, "COND": COND, "DIV": DIV,
    "EQ": EQ, "GE": GE, "GT": GT, "ISNIL": ISNIL,
    "LT": LT, "LE": LE, "MAX": MAX, "MIN": MIN, "MUL": MUL,
    "NE": NE, "NIL": NIL, "POW": POW, "PUSH": PUSH,
    "SUB": SUB, "TOS": TOS
}
...
def evaluation(e, c):
    ...
    # Create a dictionary with formal/actual parameter
    # associations.
    d = {}
...
def eval_term(e, c):
    ...
    if tok == "NUMBER" or tok == "STRING":
        ...
        raise BaseException(f"{tok, val} unexpected here")
    return retval, e, c
...
def awful_to_str(v):
    ...
    elif t == "NUMBER" or t == "ATOM": s = str(val)
    elif t == "STRING": s = '"' + val + '"'
    elif t == "LIST":
        s = "[ "
        for x in val:
